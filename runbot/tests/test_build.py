...
class Test_Build(common.TransactionCase):
    ...
    def test_build_cmd_log_db(self, mock_grep, mock_mkdir, is_file):
    ...
    @patch('odoo.addons.runbot.models.build.os.mkdir')
    @patch('odoo.addons.runbot.models.build.grep')
    def test_build_cmd_server_path_no_dep(self, mock_grep, mock_mkdir, mock_is_file):
        ...
        self.assertEqual(cmd[addons_path_pos], 'bar-d0d0caca/addons')
    ...
    @patch('odoo.addons.runbot.models.build.os.mkdir')
    @patch('odoo.addons.runbot.models.build.grep')
    def test_build_cmd_server_path_with_dep(self, mock_grep, mock_mkdir, mock_is_file, mock_is_on_remote):
        """ test that the server path and addons path """

        def is_file(file):
            self.assertIn('-master-d0d0ca/bar-dfdfcfcf/server.py', file)
            return True
